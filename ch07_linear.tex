...
But, hang on. Look at the corresponding ``number of credit hours''. These numbers don't increase by the same amount at every step: the first step goes from 3 to 4 (an increase of 1), but the second step goes from 4 to 6 (an increase of 2). This is unlike what we saw for arithmetic sequences, in which the term number increased steadily, by 1 unit every step. We're going to have to be a bit more clever to account for this!
...
Rate of change is a very important concept in mathematics, and it is a general term that doesn't just apply to straight lines. Different types of functions have different patterns that emerge in their rates of change. The study of rate of change led to the development of calculus!

But, the rate of change of a straight line is special: it's always the same. When it comes to linear relationships, the key is knowing that \textit{the rate of change for linear data is constant}. We have a special term for the rate of change in a linear relationship.

\begin{boxdef}[Slope]
\Gls{slope} is a measurement of the steepness of a line.
\end{boxdef}

For a line, the terms ``slope'' and ``rate of change'' describe the same thing: steepness. When we have a curve (like the graph of an exponential or quadratic function), we don't usually talk about its ``slope'' because the steepness changes, as we saw in \cref{sec:interpgraphs}. We can use the term ``rate of change'' with non-linear functions, as we will see in later chapters.
...
We always use the same basic approach to compute slope, and so we can derive a formula for slope. Study the picture below, showing a line through two ``generic points'' called $(x_1,y_2)$ and $(x_2,y_2)$.
...
The trickiest part about the point-slope form -- as you might anticipate -- is handling the plusses and minuses. It can be tricky to remember the $-x_1$ and the $+y_1$, so be on the lookout and \textbf{always} check your signs.

\begin{boxex}
State the slope of each of the following lines and name one point on each line.
...
If we want to move the graph of the direct variation up or down, we add something to the equation. For example, if we want to move the graph 12 units up, the equation would become $y = mx + 12$. If we want to move down 7 units, the equation would become $y = mx + \umin7$. In general, if we want to move the graph vertically $y_1$ units the equation would become $y = mx + y_1$.
...
If we want to cause \textit{both} a horizontal and vertical shift, we would do both transformations to get the point-slope form: $y=m(x-x_1)+y_1$. We can think of this transformation as ``moving the origin'' of the direct variation from $(0, 0)$ to $(x_1, y_1)$.

Yet another way to look at point-slope form is to think back to the rules we wrote for linear sequences back in \cref{sec:arithmeticseq}. Given a sequence like $4, 7, 10, 13,\dotsc$, we wrote the rule \[a_n = 4 + 3 (n-1).\] Can you see the connection to point slope form? In this rule the slope is 3 and the point is $(1,4)$. The slope is telling us the constant difference: the amount that we add on for each step forward in the sequence. The point $(1,4)$ is telling us that the first term in the sequence is 4.
...
Point-slope form is the easiest form for algebra students to use when writing the equation of a line. Once we know the slope of the line we only need one point on the line -- any point at all -- to write the equation.

However, point-slope form is a pain for algebra \textit{teachers} to grade. Why? Well, how many points are there on a line? Each one of those points can be used to write an equation for that line, which means there are infinitely many different point-slope equations for a single line! If a student gets creative about what point to use, the teacher may have to spend valuable time to determine whether the student's answer is correct or not.\footnote{Don't get any ideas.}
...
Since a linear function has a constant slope and can only have one $y$-intercept (two straight lines can't cross more than once!), a linear function has only one slope-intercept equation.

Graphing a line in slope-intercept form is exactly the same as graphing a line in point-slope form. In fact, it's easier! Figuring out the ``point'' in point-slope form can lead to some challenges with positive and negative numbers. With slope-intercept form, the ``point'' is much easier to identify. Then, we can use the slope to find a second point and graph the line.
...
\begin{boxex}
A line has a slope of 2 and passes through the point $(3, 7)$. Write the slope-intercept equation of the line.
...
&&\quad\text{the expression $(mx_1+y_1)$ is equal to $b$}\\
y &= mx + {\color{blue} b}
...
So far, we know a lot about writing equations for lines. We can find the slope of a line, we can write equations in point-slope form, and we can convert point-slope form into slope-intercept form. We now look at a final linear form, the so-called \gls{standard form}.
...
We know that an anatomically normal goat has 4 feet, and so if the pen contains $x$ goats, they account for $4x$ feet. Similarly, an anatomically normal chicken has 2 feet. If there are $y$ chickens in the pen they account for $2y$ feet. Finally, we know the total number of feet. So, we have the equation \[4x+2y=44.\]
...
Speaking of $x$-intercepts, they haven't gotten much of our attention so far. Our work with linear equations has highlighted the importance of the $y$-intercept. We'll learn much more about the significance of $x$-intercepts in future chapters.
...
This section is dedicated to a geometric connection to linear functions: parallel and perpendicular lines. Our goals for now are to learn how to tell if two lines are parallel or perpendicular and how to write equations for a line that is parallel or perpendicular to a given line. In the future (like in a geometry class), you might use concepts from this lesson to prove some geometric concepts algebraically.
...
Parallel lines never intersect. They exist in the same plane and stay the same distance apart through infinity\ldots\ and beyond! In the rectangle we drew as part of the startup exploration, opposite sides are parallel. Examine the slopes of each pair of parallel lines. What do you notice?

Algebraically, parallel lines have the \textit{same slope} and \textit{different $y$-intercepts}. Note that it is not sufficient to just look at whether two lines have the same slope. If two lines have the same slope and also have the same $y$-intercept, then they aren't actually \textit{two lines}. Those are the same line\ldots\ and you can't be parallel to yourself!\footnote{Don't start with the interpretive dance about this. You can't be parallel to yourself, no matter how you hold your arms.}

%Parallel lines will play an important role as we begin to study systems of linear equations in \cref{ch:systems}.
