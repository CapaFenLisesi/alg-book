...
A \gls{real number} is any rational or irrational number. The set of all real numbers is denoted $\R$. The symbol $\R$, naturally enough, comes from the word \textit{real}.
\end{boxeddef}
...
The \gls{opposite} of a number $x$ is the number $-x$. In other words, the opposite of a number is the number with the \textit{same absolute value}, but the \textit{opposite sign}. For example, 3 and $\umin3$ are opposites.
...
Addition and subtraction are called opposite (or inverse) operations because they ``undo'' one another. The act of adding 5 to a number can be ``undone'' by subtracting 5. On the other hand, the act of adding 5 units of matter can be undone by \textit{adding 5 units of antimatter}. In other words:
...
But what about $\umin5 \cdot 6$? What does it mean to have ``negative five groups of six''? Or even worse, what about $\umin3 \cdot \umin7$, ``negative three groups of negative seven''? Rather than try to twist the metaphor to fit these new situations, let's just admit that multiplication can \textit{not} always be represented by repeated addition.\footnote{The ``multiplication as repeated addition'' analogy breaks down when we have a negative number of groups, but also for rational and irrational numbers. What's the repeated addition problem for $\frac{2}{3} \cdot \frac{1}{2}$, or for $\sqrt{2} \cdot \sqrt{3}?$}
...
Since $x$ are $y$ are natural numbers they are both positive, and then $\umin y$ is negative. Part c. asks us to consider $x\cdot(\umin y)$. This is have a positive number times a negative number, so the product is \textit{always negative}, always less than 0.
...
If we ask ``\textit{how many} fifths?'', the numerator tells us ``\textit{three} fifths''. The word numerator is related to the word ``number'', and the numerator counts the pieces.

If we ask ``three \textit{whats}?", the denominator tells us ``three \textit{fifths}''. The word denominator is related to the word ``nominate'' (as in ``to nominate someone for president'') which means ``to name''. The denominator \textit{names} the fraction.
...
A numeric expression is completely simplified if all grouping symbols have been eliminated and operations have been evaluated. The resulting quantity is called the \textit{value} of the expression.
\end{boxedcriteria}
