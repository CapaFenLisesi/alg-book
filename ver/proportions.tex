...
%
\begin{commwork}
\frac{x}{24} &=& \frac{44}{96}
\\[\fracspace]
24\cdot\frac{x}{24} &=& 24\cdot\frac{44}{96}
& MPOE: multiply both sides by 24
\\[\fracspace]
\bcancel{24}\cdot\frac{x}{\bcancel{24}} &=& \bcancel{24}\cdot\frac{44}{\bcancel{24}\cdot4}
& simplify
\\[\fracspace]
x &=& 11
\end{commwork}
...
Of course, we might be faced with numerators and denominators that are more complicated. Here's an example with variables on both sides of the equation.
...
\begin{enumerate}
	...
	\item \onerowex{$\dfrac{15}{x}=\dfrac{25}{100}$}{We're given the percent again, only this time the question asks ``of what number'', indicating that we're seeking the whole. The unknown ends up in the denominator, but that's no problem if we remember to reciprocalize!}
\end{enumerate}
\end{boxex}
...
\begin{tikzpicture}
	\begin{axis}[
			...
			xlabel={\large Dollars},
			ylabel={\large Rupees},
			grid=both,
%			xlabel near ticks,
%			minor xtick={-300,-270,...,300},
%			ylabel near ticks,
			ytick={-300,-240,...,300},
	]
	\addplot[algcurve, green!50!black, domain=-5:5] {60*\x};
	\end{axis}
\end{tikzpicture}
\end{center}
...
Here's one more example of the kind of question associated with direct variation.
