...
%%	arg4: y-amx
{
	...
	\draw[<->,thick] (#1-0.5,0) -- (#2+0.5,0);% node[right] {\footnotesize$x$};
	\draw[<->,thick] (0,#3-0.5) -- (0,#4+0.5);% node[above] {\footnotesize$y$};
	%% Draw labels
...
%		node[below] {\footnotesize\x};
%	\foreach \y in {#3,...,#4} \draw (-0.05,\y) -- (0.05,\y)
%		node[left] {\footnotesize\y};
}
%%%--------------------------------------------------
...
\begin{boxedexplore}[Startup Exploration: Honest Senators]
...
%\subsection{A Famous Inequality}
%
...
\begin{boxedexplore}[Startup Exploration: Wire Triangle]
...
If, on the other hand, we have an exclusive inequality ($<$, $>$, and $\neq$), then the boundary point is not included in the solution set. On the graph we draw a hollow or ``open'' endpoint to show that the boundary is not part of the solution. We call the boundary an \inlinedef{exclusive boundary}.
...
\begin{boxeddefex}[Properties of Order: Addition and Subtraction]
...
\begin{boxeddefex}[Properties of Order: Multiplication and Division]
...
To accomplish this, we substitute the values $x=4$ and $y=5$ into the given inequality, and then see whether the resulting inequality is true or false:
\[\begin{aligned}
y &\leq -2x+6
...
%		\addplot[algpoints, red] coordinates {(4,5)};
%		\addplot[algpoints, white] coordinates {(-3,2)};
		\fill[nearly transparent, blue] (axis cs:-2,17/3,0) -- (axis cs:-2,17,0) -- (axis cs:17,17,0) -- (axis cs:17,-2/3,0) -- cycle ;
	\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{boxedexplore}[Startup Exploration: Evil Vegan Appliances]
...
There are multiple techniques for optimization. The approach we will learn here is called \inlinedef{linear programming}. Real-world linear programming problems have many variables, sometimes numbering in the millions. Since we're only in algebra 1, we'll stick with just a few variables.

\subsection{The Process of Linear Programming}
