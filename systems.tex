...
When finding an intersection point using a calculator, we can use the \calc{trace} function on the calculator. But, \calc{trace} can only give us a decimal approximation, and this could be a problem for some solutions. (For example, a solution that is not whole numbers or a fraction with ``convenient'' denominator.)

Using \calc{trace} is good for approximating a solution, but then we can use an alternative way: looking at the table. Here, we can search the table for a place where \calc{y1} and \calc{y2} are the same for a particular value of \calc{x}.

For the t-shirt example, go to the \calc{table} screen on your calculator, and scroll to where \calc{x} is 36. You should see that both \calc{y1} and \calc{y2} are 432. This will be how the intersection point will show up in a table.
