...
\chapquote{The tremendous power of the simplex method is a constant surprise to me.}{George Dantzig, American mathematician}
...
How do we describe mathematically unequal relationships? What rules govern these relationships? How to we represent such relationships visually? These questions are at the heart of this chapter. By the end, we will have developed the simple idea of ``inequality'' into a sophisticated tool for finding an optimal solution in certain complex situations where many choices are possible (the \textit{simplex method} mentioned in the quote above).
...
There are multiple techniques for optimization. The approach we will learn here was first called the \textit{simplex method}, and has now come to be called \inlinedef{linear programming}.\footnote{More on the interesting history of linear programming at the end of this section!} Real-world linear programming problems have many variables, sometimes numbering in the millions. Since we're only in Algebra 1, we'll stick with just a few variables.
