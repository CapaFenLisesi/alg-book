...
\begin{figure}[!htbp]
	...
	\begin{tikzpicture}[scale=0.4]
		...
		\draw (13.5,0) node[left]{$x$-axis};
		\draw (-13.5,0) node[right] {};
		\draw[<->,thick] (0,-10.5) -- (0,10.5) node[above]{$y$-axis};
...
The axes chop the plane into four regions called \glspl{quadrant}, which are numbered starting in the upper right and moving counter-clockwise (as shown in the figure). The signs in parentheses indicate the signs of $x$- and $y$-coordinates in each quadrant. The $x$-coordinates of the points in Quadrants I and IV are positive, while points in Quadrants II and III have $x$-coordinates that are negative. Points with positive $y$-coordinates lie in Quadrants I and II, while points with negative $y$-coordinates fall in Quadrants III and IV.

\begin{boxedexplore}[Startup Exploration: Pizza Intersections]
...
%This means instead of writing rules expressing $a_n$ in terms of $n$, we'll write rules expressing $y$ in terms of $x$.
%This is a geometric sequence, with the recursive rule ``START~=~1, NEXT~=~NOW~$\cdot~2$''. 
...
\begin{minipage}[c]{0.5\textwidth }
	...
	\hline
	\text{\footnotesize Term No.} & \text{\footnotesize Term Value} & \text{\footnotesize Coord. Pair}\\
	x & y & (x,y)\\\hline
	1 & 1 & (1,1)\\
	2 & 2 & (2,2)\\
	3 & 4 & (3,4)\\
...
\begin{minipage}[c]{0.5\textwidth }
	...
	\begin{tikzpicture}[xscale=1,yscale=0.15]
		%% grid setup
		\draw[very thin, color=gray!25, ystep=5] (0,0) grid (7, 42);
		\draw[->,thick] (0,0) -- (7,0); % node[right]{$x$-axis};
		\draw[->,thick] (0,0) -- (0,42); % node[above]{$y$-axis};
		\foreach \x in {0,...,6} \draw (\x,0.05) -- (\x,-0.05) node[below] {\footnotesize\x};
		\foreach \y in {0,5,...,40} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[fill=blue,blue] (1,1) circle (.1cm and 0.66cm);
		\draw[fill=blue,blue] (2,2) circle (.1cm and 0.66cm);
		\draw[fill=blue,blue] (3,4) circle (.1cm and 0.66cm);
		\draw[fill=blue,blue] (4,8) circle (.1cm and 0.66cm);
		\draw[fill=blue,blue] (5,16) circle (.1cm and 0.66cm);
		\draw[fill=blue,blue] (6,32) circle (.1cm and 0.66cm);
	\end{tikzpicture}
\end{minipage}
\medskip
...
\begin{minipage}[c]{0.5\textwidth }
	...
	\begin{tikzpicture}[xscale=1,yscale=0.15]
		...
		\foreach \y in {0,5,...,40} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[fill=red,red] (1,8) circle (.1cm and 0.66cm);
		\draw[fill=red,red] (2,13) circle (.1cm and 0.66cm);
		\draw[fill=red,red] (3,18) circle (.1cm and 0.66cm);
		\draw[fill=red,red] (4,23) circle (.1cm and 0.66cm);
		\draw[fill=red,red] (5,28) circle (.1cm and 0.66cm);
		\draw[fill=red,red] (6,33) circle (.1cm and 0.66cm);
	\end{tikzpicture}
\end{minipage}
\medskip
...
\begin{minipage}[c]{0.5\textwidth }
	...
	\begin{tikzpicture}[xscale=1,yscale=0.15]
		...
		\foreach \y in {0,5,...,40} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[fill=violet,violet] (1,1) circle (.1cm and 0.66cm);
		\draw[fill=violet,violet] (2,4) circle (.1cm and 0.66cm);
		\draw[fill=violet,violet] (3,9) circle (.1cm and 0.66cm);
		\draw[fill=violet,violet] (4,16) circle (.1cm and 0.66cm);
		\draw[fill=violet,violet] (5,25) circle (.1cm and 0.66cm);
		\draw[fill=violet,violet] (6,36) circle (.1cm and 0.66cm);
	\end{tikzpicture}
\end{minipage}
\medskip
...
\begin{boxedexplore}[Startup Exploration: Don't Take All Year]
...
\begin{boxedexplore}[Startup Exploration: Extending Our Sequences]
...
\begin{center}
	\begin{tikzpicture}[xscale=0.7,yscale=0.14]
		%% grid setup
		\draw[very thin, color=gray!25, ystep=2] (-5.5,-25) grid (5.5, 29);
		\draw[<->,thick] (-5.5,0) -- (5.5,0); % node[right]{$x$-axis};
		\draw[<->,thick] (0,-25) -- (0,30); % node[above]{$y$-axis};
		\foreach \x in {-5,...,-1,1,2,3,4,5} \draw (\x,0.05) -- (\x,-0.05) node[below] {\footnotesize\x};
		\foreach \y in {-24,-20,...,28} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[fill=red,red] (-5,-22) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (-4,-17) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (-3,-12) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (-2,-7) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (-1,-2) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (0,3) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (1,8) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (2,13) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (3,18) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (4,23) circle (.1cm and 0.5cm);
		\draw[fill=red,red] (5,28) circle (.1cm and 0.5cm);
	\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}[xscale=0.7,yscale=0.14]
		...
		\foreach \y in {-24,-20,...,28} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[<->,very thick,color=red,domain=-5:5] plot (\x,5*\x+3); 
	\end{tikzpicture}
\end{center}

The rules below correspond to the other two sequences we studied in \cref{sec:vizsequence}. Under each rule is the graph that is created when we plot the rule over $\R$.
...
\begin{minipage}[c]{0.5\textwidth }
	...
	\begin{tikzpicture}[xscale=0.6,yscale=0.12]
		...
		\foreach \y in {-24,-20,...,28} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[<->,very thick,color=blue,domain=-5:4.5] plot (\x,2^\x); 
	\end{tikzpicture}
...
\begin{minipage}[c]{0.5\textwidth }
	...
	\begin{tikzpicture}[xscale=0.6,yscale=0.12]
		...
		\foreach \y in {-24,-20,...,28} \draw (-0.05,\y) -- (0.05,\y) node[left] {\footnotesize\y};
		%% points
		\draw[<->,very thick,color=violet,domain=-5:5] plot (\x,\x*\x); 
	\end{tikzpicture}
\end{minipage}

Are you surprised by these two graphs? Back in \cref{sec:vizsequence}, the graphs of these two sequence looked similar. But, we were only looking at the first quadrant! Their graphs are very different for negative values of $x$.
...
\addtodoitem{Criteria for high quality graphs.}

%\subsection{Criteria for High Quality Graphs}
%
%In algebra we make a distinction between ``sketching'' and ``graphing''. A sketch is a picture, a quick drawing, and it doesn't need to be super accurate. A sketch can be scribbled on a napkin or a piece of notebook paper.
%
%On the other hand, a proper graph is meant to communicate something to the viewer. A graph must be accurately made on graph paper. The most important features of whatever you're graphing---a sequence, a function, a plot of a data set---must be clear, accurate, and neatly represented.
%
%Here are some guidelines for creating high-quality graphs.
%
%* * * * * * * * * * 
%High quality graphs are drawn on graph paper.
%
%Graphs of equations do not have to take up an entire piece of graph paper.
%
%Use a straightedge when drawing the coordinate axes.
%
%High quality graphs use an appropriate scale for the data and the size of the grid.
%
%Choose a scale that fits the data and ensures that your graph shows all important features of the curve. Scales for the $x$- and $y$- axes can be different, and in some cases must be different, so this can take a little bit of planning.
%
%Note that the scale must be the same for the whole length of an axis, and cannot ``jumps'' or ``breaks''. Changes in the scale like these will distort the shape of the graph, which defeats the purpose!
%
%High quality graphs are clearly labeled.
%> For graphs of equations, this means labeling the axes $x$ and $y$. When plotting data, this means more informative names like ``time'' or ``distance''.
%
%High quality graphs have an appropriate scale that is clearly labeled.
%> Graphs, no matter how beautiful, mean nothing without a scale. A line is just a line! We can't tell \textit{what line it is} without a scale!
%
%In addition scatter plots need to be large enough for you to draw lines/curves of best fit.
%
%High quality graphs are mindful of placement? 
%
%The origin does not necessarily need to be in the center of the grid. For example, if we are plotting data that includes no negative values, we only need the first quadrant of the graph, and so the origin can be in the lower left-hand corner of the graph.
%
%To connect or not to connect? Data points in a scatter plot are not connected. Data points in the graph of an equation are connected.
%
%When graphing an equation by plotting data points, connect non-linear data points with a smooth curve (not individual line segments). No pointy parabolas! If we use a straight line to connect points, then we are telling our viewers that the data in between the points is linear, which may not be the case.
%
%When graphing a line, use a ruler or straightedge. Don't try to ``free-hand'' draw a straight line. It rarely works.
%
%Use arrows for anything that goes forever
%
%This includes axes, lines, and curves. Open or closed dots are used for end points. Now, open and closed end points mean two different things. You can't use them interchangeably. Closed dots are inclusive and open dots are exclusive.
...
\begin{boxedexplore}[Startup Exploration: Water Consumption]
...
%\begin{boxedexplore}[Warm-up: Guess Your Weight]
...
\begin{center}
% (height, weight)
\begin{tikzpicture}[scale=0.9, domain=150:200]
	\begin{axis}[xlabel={Height (cm)}, ylabel={Weight (kg)}]
...
\begin{tikzpicture}[scale=0.9, domain=150:200]
	\begin{axis}[xlabel={Height (cm)}, ylabel={Jersey Number}]
	\addplot[scatter, only marks, scatter src=\thisrow{class}]
      table[x=x,y=y] {
	x		y		class
	188		39		1
...
Similarly, imagine a graph showing ``time'' as the independent variable and ``number of hockey players on the ice'' as the dependent variable. In this case, the $y$-values would have no ''in-betweens''. There could be 11 players or 12 players on the ice, but never 11.5 players.
...
\begin{tikzpicture}[scale=0.7]
	...
	\foreach \x in {0,...,6}
		\draw (\x,0.1) -- (\x,-0.1);
	\draw (3, -0.75) node[below] {Distance from home};
	\draw (3, -1.5) node[below] {(continuous data)};
	%% y-axis
	\foreach \y in {0,...,5}
		\draw (-0.1,\y) -- (0.1,\y);
	%% graph
	\draw[very thick, blue, rounded corners] (0,0) .. controls (1,4.5) .. (2,2) .. controls (3,0) .. (4,2) .. controls (5,3.5) .. (6,0);
\end{tikzpicture}
\end{minipage}
%%
\begin{minipage}{0.5\textwidth}
...
\begin{tikzpicture}[scale=0.7]
	...
	\foreach \x in {0,...,6}
		\draw (\x,0.1) -- (\x,-0.1);
	\draw (3, -0.75) node[below] {Number of customers};
	\draw (3, -1.5) node[below] {(discrete data)};
	%% y-axis
	\foreach \y in {0,...,5}
		\draw (-0.1,\y) -- (0.1,\y);
	%% graph
	\draw[very thick, blue] (0,1) -- (2,1);
	\draw[very thick, blue] (2,2) -- (3,2);
	\draw[very thick, blue] (3,4) -- (5,4);
...
\begin{boxedexplore}[Startup Exploration: Yeardleigh's Submersible]
